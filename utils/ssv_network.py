...
class SSVNetwork:
  ...
  def register_validator(self, publicKey, operatorIds, sharesPublicKeys, sharesEncrypted, amount,account_address):
    self.contract.functions.registerValidator(publicKey, operatorIds, sharesPublicKeys, sharesEncrypted,
...
class SSVToken:
  ...
  def approve(self, address, amount, account_address):
    return self.contract.functions.approve(address, amount).buildTransaction(
      {"from": account_address, "maxFeePerGas": 10 ** 12})
